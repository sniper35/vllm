...
class GPUModelRunner(
    ...
    def execute_model(
        ...
        # Set detect/point embed data before forward (Moondream3).
        # With PP>1 the last rank updates pending_embed_* in
        # compute_logits / update_after_sample during the *previous*
        # step.  Broadcast those values to all ranks so the first rank
        # (which owns the embedding layer) can inject Fourier-encoded
        # coordinate/size embeddings before forward.
        dp_mgr = getattr(self.model, "detect_point_manager", None)
        if dp_mgr is not None and dp_mgr.has_active_requests():
            pp = get_pp_group()
            if pp.world_size > 1:
                self._sync_dp_pending_embeds(dp_mgr, num_reqs, pp)
            dp_embed_data: dict[int, dict] = {}
            for i in range(num_reqs):
                rid = self.input_batch.req_ids[i]
                dp_st = dp_mgr.get_state(rid)
                if dp_st is not None and dp_st.pending_embed_type is not None:
                    pos = int(logits_indices[i])
